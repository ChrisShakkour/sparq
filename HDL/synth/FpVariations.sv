...
module FpVariations
  #(
   ...
   (
    input logic 		      clk,
    input logic 		      rstn,
    input logic 		      set_zero [EXP_MAX:EXP_MIN][MAN_MAX:MAN_MIN],
    input logic 		      set [EXP_MAX:EXP_MIN][MAN_MAX:MAN_MIN],
    input logic [W_FP_THRESHOLD-1:0]  set_value [EXP_MAX:EXP_MIN][MAN_MAX:MAN_MIN],
   ...
   logic 			      set_zero_reg [EXP_MAX:EXP_MIN][MAN_MAX:MAN_MIN];
   logic [W_FP_THRESHOLD-1:0] 	      set_reg  [EXP_MAX:EXP_MIN][MAN_MAX:MAN_MIN];
   logic [W_FP_THRESHOLD-1:0] 	      set_value_reg  [EXP_MAX:EXP_MIN][MAN_MAX:MAN_MIN];
   ...
   generate
	 ...
	 for(mantissa_i=MAN_MIN; mantissa_i<=MAN_MAX; mantissa_i++) begin : mantissa_col
	    ...
	    if(W_FP_NUMBER < W_FP_THRESHOLD) begin : fp_mult_inst_enable
	       FpProccessingElement #(.W_MANTISSA(W_MANTISSA), .W_EXPONENT(W_EXPONENT))
	       FpProccessingElement_inst
		 (
		  .clk		(clk),
		  .rstn		(rstn),
		  .set_zero	(set_zero_reg	[exponent_i][mantissa_i]),
		  .set		(set_reg	[exponent_i][mantissa_i]),
		  .set_value	(set_value_reg	[exponent_i][mantissa_i]),
		  ...
		  if(~rstn) set_zero_reg[exponent_i][mantissa_i] <= '0;
		  else      set_zero_reg[exponent_i][mantissa_i] <= set_zero[exponent_i][mantissa_i];
	       end

	       always_ff @(posedge clk or negedge rstn) begin
		  if(~rstn) set_reg[exponent_i][mantissa_i] <= '0;
		  else      set_reg[exponent_i][mantissa_i] <= set[exponent_i][mantissa_i];
	       end

	       always_ff @(posedge clk or negedge rstn) begin
		  if(~rstn) set_value_reg[exponent_i][mantissa_i] <= '0;
		  else      set_value_reg[exponent_i][mantissa_i] <= set_value[exponent_i][mantissa_i];
	       end
